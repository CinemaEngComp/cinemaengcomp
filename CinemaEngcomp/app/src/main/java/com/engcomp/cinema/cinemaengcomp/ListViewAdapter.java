...
class ListViewAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        nome_filme.setText(resultp.get(Cartaz.NOME_FILME));
        genero_filme.setText(resultp.get(Cartaz.GENERO_FILME));
        duracao_filme.setText(resultp.get(Cartaz.DURACAO_FILME));
        classificacao_filme.setText(resultp.get(Cartaz.CLASSIFICACA0_FILME));
        sinopse_filme.setText(resultp.get(Cartaz.SINOPSE_FILME));

        // Capture position and set results to the ImageView
        // Passando url da imagem para a classe ImageLoader.class
        imageLoader.DisplayImage(resultp.get(Cartaz.CAMINHO), caminho);
        ...
        itemView1.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View arg0) {
                ...
                intent.putExtra("nome_filme", resultp.get(Cartaz.NOME_FILME));
                intent.putExtra("genero_filme", resultp.get(Cartaz.GENERO_FILME));
                intent.putExtra("duracao_filme", resultp.get(Cartaz.DURACAO_FILME));
                intent.putExtra("classificacao_filme", resultp.get(Cartaz.CLASSIFICACA0_FILME));
                intent.putExtra("sinopse_filme", resultp.get(Cartaz.SINOPSE_FILME));
                intent.putExtra("email", resultp.get("email"));
                intent.putExtra("caminho", resultp.get(Cartaz.CAMINHO));
                intent.putExtra("codigo_filme", resultp.get(Cartaz.CODIGO_FILME));
