...
dhtmlxCalendarObject.prototype.show = function(){
	...
	if(_isIE){//add iframe under calendard in IE (IE 6 issue with select box fix)
		...
		if(this.parent.style.position == "absolute" && this.ifr==undefined){
			...
			this.ifr.style.top = this.parent.offsetTop;
			this.ifr.style.left = this.parent.offsetLeft;
			this.ifr.style.width = this.parent.offsetWidth;
			this.ifr.style.height = this.parent.offsetHeight;
...
//	alert( this.style + (newSkin?'_':"") + newSkin + (this.minimized?" " + this.style + (newSkin?'_' + newSkin:"") + "_minimized":""));
//	if (newSkin)
...
dhtmlxCalendarObject.prototype.setFormatedDate = function(dateformatarg, date){
	...
	var outputDate = new Date(2008, 0, 0);//default date 2008.01.01
