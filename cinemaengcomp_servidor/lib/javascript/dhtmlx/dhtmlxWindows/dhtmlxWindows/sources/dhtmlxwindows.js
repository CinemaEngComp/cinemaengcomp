...
function dhtmlXWindows() {
	...
	*/
	...
	this._redrawWindow = function(win) {
		...
		if (w < 0) { w = 0; }
		if (h < 0) { h = 0; }
		p.childNodes[0].childNodes[0].childNodes[0].childNodes[1].childNodes[0].style.width = w + "px";
		p.childNodes[0].childNodes[0].childNodes[0].childNodes[1].childNodes[0].style.height = h + "px";
		//
	}
...
*/
	...
	this._maximizeWindow = function(win) {
		...
		win.w = (win.maxW == "auto" ? parseInt(this.vp.style.width) : win.maxW);
		win.h = (win.maxH == "auto" ? parseInt(this.vp.style.height) : win.maxH);
	...
	this._closeWindow = function(win) {
		...
		// closing
		for (a in win.btns) { this._removeButtonGlobal(win, a, win.btns[a]); }
		...
		*/
		var latest = { "zi": 0 };
		for (a in this.wins) { if (this.wins[a].zi > latest.zi) { latest = this.wins[a]; } }
	...
	*/
	...
	this._bringOnTopAnyStickedWindows = function() {
		var wins = new Array();
		for (a in this.wins) { if (this.wins[a]._isSticked) { wins[wins.length] = this.wins[a]; } }
