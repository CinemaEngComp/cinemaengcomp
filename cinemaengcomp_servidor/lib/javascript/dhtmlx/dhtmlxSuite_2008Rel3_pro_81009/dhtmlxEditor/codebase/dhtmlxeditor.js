...
function dhtmlXEditor(id, skin) {this.skin = (skin!=null?skin:"dhx_blue");this._tbH = 24;if (this.skin == "standard"){this._tbH = 26};this.iconsPath = "codebase/imgs/";this.setIconsPath = function(path) {this.iconsPath = path};this._genStr = function(w) {var s = "";var z = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";for (var q=0;q<w;q++){s = s + z.charAt(Math.round(Math.random() * z.length))};return s};this.init = function() {this._attachToolbar()};this.toolbarId = "dhxToolbar"+this._genStr(6);this.base = (typeof(id)=="object"?id:document.getElementById(id));while (this.base.childNodes > 0){this.base.removeChild(this.base.childNodes[0])};this.base.innerHTML = "<table border='0' cellspacing='0' cellpadding='0' style='width:100%;height:100%;table-layout:fixed;'>"+
 ...
 "</td></tr>"+
 "</table></td></tr>"+
 "</table>";this.editor = document.createElement("IFRAME");this.editor.className = "dhxeditor_mainiframe_"+this.skin;this.editor.frameBorder = 0;if (_isOpera){this.editor.scrolling = "yes"};this._td1 = this.base.childNodes[0].childNodes[0].childNodes[0].childNodes[0];this._td2 = this.base.childNodes[0].childNodes[0].childNodes[1].childNodes[0];this._td2cont = this.base.childNodes[0].childNodes[0].childNodes[1].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0];this._cont = this.base.childNodes[0].childNodes[0].childNodes[1].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0].childNodes[0];this._cont.appendChild(this.editor);this.edWin=this.editor.contentWindow;this.edDoc=this.edWin.document;this.edWin=this.editor.contentWindow;var content=this.edDoc;content.open("text/html", "replace");if (_isOpera){content.write("<html><head><style> html, body {overflow:auto;padding:0px;padding-left:1px !important;height:100%;margin:0px;font-family:Tahoma;font-size:10pt;background-color:#ffffff};</style></head><body contenteditable='true'></body></html>")}else {content.write("<html><head><style> html, body {overflow-x:auto;overflow-y:scroll;padding:0px;padding-left:1px !important;height:100%;margin:0px;font-family:Tahoma;font-size:10pt;background-color:#ffffff};</style></head><body contenteditable='true'></body></html>")};content.close();this.dhx_Event();this.edDoc.designMode='On';var that = this;dhtmlxEvent(this.edDoc, "click", function(e){var ev = e||window.event;var el = ev.target||ev.srcElement;that.showInfo(el)})
 if(_isOpera)dhtmlxEvent(this.edDoc, "mousedown", function(e){var ev = e||window.event;var el = ev.target||ev.srcElement;that.showInfo(el)})
 dhtmlxEvent(this.edDoc, "keyup", function(e){var ev = e||window.event;var key = ev.keyCode;var el = ev.target||ev.srcElement;if((key==37)||(key==38)||(key==39)||(key==40))
 that.showInfo(el)})
 
 this.adjustSize = function() {this._td1.style.height = this._tbH+"px";this._td2.style.height = this.base.offsetHeight-this._tbH+"px";this._td2cont.style.height = this.base.offsetHeight - this._tbH - (_isIE?5:5) +"px";this._cont.style.height = this._td2cont.style.height;this.editor.style.height = this._td2cont.style.height};this.adjustSize();this.style = null;if(_isFF)this.edDoc.execCommand("useCSS",false,true);this.applyBold = function(){this.runCommand("Bold")};this.applyItalic = function(){this.runCommand("Italic")};this.applyUnderscore = function(){this.runCommand("Underline")};this.applyStrikethrough = function(){this.runCommand("StrikeThrough")};this.alignLeft = function(){this.runCommand("JustifyLeft")};this.alignRight = function(){this.runCommand("JustifyRight")};this.alignCenter = function(){this.runCommand("JustifyCenter")};this.alignJustify = function(){this.runCommand("JustifyFull")};this.applySub = function(){this.runCommand("Subscript")};this.applySuper = function(){this.runCommand("Superscript")};this.applyH1 = function(){this.runCommand("FormatBlock","<H1>")};this.applyH2 = function(){this.runCommand("FormatBlock","<H2>")};this.applyH3 = function(){this.runCommand("FormatBlock","<H3>")};this.applyH4 = function(){this.runCommand("FormatBlock","<H4>")};this.createNumList = function(){this.runCommand("InsertOrderedList")};this.createBulList = function(){this.runCommand("InsertUnorderedList")};this.increaseIndent = function(){this.runCommand("Indent")};this.decreaseIndent = function(){this.runCommand("Outdent")};this.getContent = function(){if(!this.edDoc.body)return "";else{if(_isFF){return this.edDoc.body.innerHTML.replace(/<br>$/,"")
 };return this.edDoc.body.innerHTML}};this.setContent = function(str){if(this.edDoc.body){this.edDoc.body.innerHTML = str;this.callEvent("onContentSet",[])}else{var that = this;dhtmlxEvent(this.edWin, "load", function(e){that.setContent(str)})
 }};this.setContentHTML = function(url){(new dtmlXMLLoaderObject(this._ajaxOnLoad,this,false,true)).loadXML(url)};this._ajaxOnLoad = function(obj,a,b,c,loader){if(loader.xmlDoc.responseText)obj.setContent(loader.xmlDoc.responseText)};this.runCommand = function(name,param){if(arguments.length < 2)param = null;this.edDoc.execCommand(name,false,param);if(_isIE)this.edWin.focus()};this.getSelectionBounds = function(){var range, root, start, end;if(this.edWin.getSelection){var selection = this.edWin.getSelection();range = selection.getRangeAt(selection.rangeCount-1);start = range.startContainer;end = range.endContainer;root = range.commonAncestorContainer;if(start.nodeName == "#text")root = root.parentNode;if(start.nodeName == "#text")start = start.parentNode;if (start.nodeName.toLowerCase()== "body") start = start.firstChild;if(end.nodeName == "#text")end = end.parentNode;if (end.nodeName.toLowerCase()== "body") end = end.lastChild;if(start == end)root = start;return {root: root,
 start: start,
 end: end
 ...
 };this.showInfo = function(el){var el = (this.getSelectionBounds().root)?this.getSelectionBounds().root : el;if(!el)return
 ...
 case "applyH3":
 case "applyH4":
 this._checkH(itemId);break}};this._doOnBeforeStateChange = function(itemId, state) {if ((itemId == "alignLeft" || itemId == "alignCenter" || itemId == "alignRight" || itemId == "alignJustify")&& state == true) {return false};return true};var that = this;this.attachEvent("onFocusChanged", function(state){that._doOnFocusChanged(state)});this.tb.attachEvent("onClick", function(id){that._doOnToolbarClick(id)});this.tb.attachEvent("onStateChange", function(id,st){that._doOnStateChange(id,st)});this.tb.attachEvent("onBeforeStateChange", function(id,st){return that._doOnBeforeStateChange(id,st)})};
//v.2.0 build 81107
