...
dhtmlXGridObject.prototype.goHOME=function(rowID,celIndex,valor) {
	this.showRow(this.getRowId(0));	
}
dhtmlXGridObject.prototype.goEND=function(rowID,celIndex,valor) {
//	this.showRow(this.getRowId(this.rowCount()-1));	
	this.selectCell(this.getSelectedRowId(),this.getColumnsNum()-1);
//		console.log('goEND');
...
dhtmlXGridObject.prototype.setCellByID=function(rowID,celIndex,valor) {
	this.cells(rowID,celIndex).setValue(valor);	
}
...
dhtmlXGridObject.prototype.SetXMLDataField=function(name,row_id,value) {
//	return this.cells(this.getRowId(rowID),celIndex).getValue();
//	console.log('SetXMLDataField: name='+name+' /row_id='+row_id+' /value='+value);
	this.setUserData(row_id,name,value);
}
dhtmlXGridObject.prototype.GetXMLDataField=function(name,row_id) {
//	return this.cells(this.getRowId(rowID),celIndex).getValue();
//	console.log('GetXMLDataField: name='+name+' /row_id='+row_id);
	return this.getUserData(row_id,name);
}
dhtmlXGridObject.prototype.SetXMLDataFieldIndex=function(name,row_index,value) {
//	return this.cells(this.getRowId(rowID),celIndex).getValue();
//	console.log('SetXMLDataFieldIndex: name='+name+' /row_index='+row_index+' /value='+value+' / row_id='+this.getRowId(row_index));
	this.setUserData(this.getRowId(row_index),name,value);
}
dhtmlXGridObject.prototype.GetXMLDataFieldIndex=function(name,row_index) {
//	return this.cells(this.getRowId(rowID),celIndex).getValue();
//	console.log('GetXMLDataFieldIndex: name='+name+' /row_index='+row_index+' / row_id='+this.getRowId(row_index));
	return this.getUserData(this.getRowId(row_index),name);
}
var lastinsert_rowid='';
var lastinsert_rowid_index=400;
dhtmlXGridObject.prototype.insertRow=function(valores) {
	if ( valores == null ){
		valores="";
	}
	lastinsert_rowid = lastinsert_rowid_index++;//(new Date()).valueOf();
	this.addRow(lastinsert_rowid,valores);
	return lastinsert_rowid;
}
//dhtmlXGridObject.prototype.deleteRow=function(ind1) {
...
ebadhtmlx_log(rC);
	...
	var onafterinsert=$("#"+objEBA.id).attr('onafterinsert');
	...
	var saveHandler=$("#"+objEBA.id).attr('saveHandler');
		...
		ebadhtmlx_log('C1');
		ebadhtmlx_log(tabela_colunas);
ebadhtmlx_log(tabela_colunas.length);
	// Todas Colunas
	for(i=0;i<tabela_colunas.length;i++) {
		...
		if ('NUMBER'==tabela_colunas[i][2]) {
			...
			if ( (""==tabela_colunas[i][6]) || (null==tabela_colunas[i][6])  ){
				tabela_colunas[i][6]="0";	
			}
			...
			atributos+=' type="'+tipo+'" align="right" ';
		} 
		if ('CHECKBOX'==tabela_colunas[i][2]) {
			atributos+=' type="nucci_simnao" ';
		} 
...
//	}
	...
	if(typeof onafterinsert === 'undefined') {
		onafterinsert='';
	} else {
		var setEditCell='gridatual.attachEvent("onRowAdded",function (rId) {  '+onafterinsert+' });';
//		console.log(setEditCell);
		eval(setEditCell);				
	}
...
//onafterinsert
	...
	for(i=0;i<tabela_colunas.length;i++) {
		...
		if (""!=onvalidate[i]) {
			if ('CHECKBOX'==tabela_colunas[i][2]) {
				functionEditCell+=' if (1==stage) { if ('+i+'==cellInd) { retorno='+onvalidate[i]+'; '+onmodified+'; return retorno;	 } }';			
			} else if ('LISTBOX'==tabela_colunas[i][2]) {
...
//			if ('LISTBOX'==tabela_colunas[i][2]) {
//				functionEditCell+='  if ( (1==stage) && ('+i+'==cellInd)) { console.log("show row");'+objEBA.id+'.selectCell(rowId,cellInd,false,true,true);	 }  ';			
//			}
      ...
      if(typeof(obj)!='object')
         obj = document.getElementById(obj);
            ...
            for (var i=0; i<hrow.cells.length; i++){
                ...
                ze+=(ze?",":"")+(hrow.cells[i].getAttribute("sort")||"na");
