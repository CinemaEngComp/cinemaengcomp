...
class EBAGetHandler
{
	...
	function DefineRecordFieldValue($ColName, $DataValue)
	{
		if ($this->Status == 2)
		{
			...
			if ($EBAKey !== FALSE)
			{
				$this->Recordset[$EBAKey] = htmlentities($DataValue);
			}
		}
	}
	...
	function DefineRecordFieldValue2($ColName, $DataValue)
	{
		if ($this->Status == 2)
		{
			// REMEMBER TO PREFORMAT THE STRING
			...
			if ($EBAKey !== FALSE)
			{
				$this->Recordset[$EBAKey] = ($this->mychecksintaxe_GRID($DataValue));
			}
		}
	}
}
...
class EBASaveHandler
{
	...
	function ReturnInsertField($RecordNumber, $FieldName = "")
	{
		...
		if (($index !== FALSE) || ($FieldName == ""))
		{
			...
			{
				// FCV 30/04/2011 - Coloquei um estado para diferenciar os campos normais dos campos compostos...
