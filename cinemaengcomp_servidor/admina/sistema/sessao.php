            ...
            <div class="col-lg-12">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>Grid de Sessoes<small class="m-l-sm">Gerencie suas Sessoes</small></h5>
                    </div>
...
?>                    
                    <select class="form-control m-b" name="filme" id="select_filme" onchange="DoFilmeSelect()">
                                    <option value="0">Selecione um filme</option>

<?  while($rowcombo = mysql_fetch_array($resultcombo)){ ?>
                                    <option value="<?=$rowcombo['codigo_filme']?>"><?=$rowcombo['nome_filme']?></option>
<?  } ?>
                                    </select>
                        </h1>
...
//      mygrid.imgURL = "/php/lib/javascript/dhtmlx/dhtmlxSuite_2008Rel3_pro_81009/dhtmlxGrid/codebase/imgs/icons_greenfolders/";
        ...
        mygrid.setHeader("ID,Filme, Sala, Data Sessao, Horario, FilmeCod");
        mygrid.setInitWidths("50,150,100,80,80,0")
        mygrid.setColAlign("left,left")
        mygrid.setColTypes("ro,ro,coro,edn,edtxt,edtxt,ro");
...
//      mygrid.enableSmartXMLParsing(true);
        ...
        mygrid.loadXML("sessao_get.php?f=0");
        
        
        myDataProcessor = new dataProcessor("sessao_save.php"); 
        ...
        comboconta5.save();
        comboconta5.put('1','1-Normal');
        comboconta5.put('2','2-3D');
        comboconta5.put('3','3-4D');
        comboconta5.put('4','4-IMAX');
...
function DoFilmeSelect() {

    var filme=$('#select_filme').val();
    console.log('DoFilmeSelect='+filme);
    mygrid.clearAll();  
    mygrid.loadXML("sessao_get.php?f="+filme);

}
function doIncluirLinha() {
//  mygrid.addRow(123,"text1,text2",1);
    var filme=$('#select_filme').text();
    var filme = $("#select_filme option:selected").text();

    var filmeval=$('#select_filme').val();

    if (null==mygrid.getSelectedId()) {
        mygrid.addRow('NOVO',['NOVO',filme,'','','',filmeval]);
    } else {
        mygrid.addRowAfter((new Date()).valueOf(),['NOVO',filme,'','','',filmeval],mygrid.getSelectedId());
    }
}
